# Repo requirements
This repo uses Python 3.8. We recommend working in a new conda environment with Python 3.8 and the requirements in the requirements.txt.
To get started:
...
1. Save data in the `cache` folder. Seizure events should be stored in `cache/seizure_events/`, one `.json` file per patient. Filenames should match the patient ID (e.g., `P1.json` for patient `P1`).
...
The `-p` argument is what makes this code run pseudo-prospectively. If you don't add this flag, the algo will use all events to train and will produce a future forecast from time of running.
...
Note that the patient names should correspond to the name of the files in /cache (not including the .json file extension)
...
After running the forecast in pseudo-prospective mode, you can execute `python src/validation/validate_all.py` from your terminal.
